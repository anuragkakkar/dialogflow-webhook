...
def productDetails(id_order):
    print(id_order)  #2QMNZ6GJQ4BFY9TXMZXJAE6E5YLNRFKZ
    url = "http://35.196.130.106/api/orders/"+id_order+"?output_format=JSON"
    print(url)
    response = requests.get(url, auth=HTTPBasicAuth('2QMNZ6GJQ4BFY9TXMZXJAE6E5YLNRFKZ', ''))
    print(response)
    if response.status_code in [200]:
        ...
        return "Your Product Name:{} and\n Price:{} and\n Order by:{}".format(product_name,product_paid_amount,order_by_payment)
    else:
        return "Sorry, no order found for "+str(id_order)
